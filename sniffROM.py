...
with open(args.input_file, 'rb') as infile:
	...
	for row in reader:
		...
		if row[1].isdigit():   # ignores the first header line
			...
			if new_packet_id > packet_id:    # IF WE GOT A NEW COMMAND INSTANCE (new Packet ID according to Saleae SPI analyzer)
				#print 'old pkt id was {0} and new pkt id is {1}'.format(packet_id, new_packet_id)
				packet_id = new_packet_id
				new_command = mosi_data
				
				
				# the new packet ID tells us the last command is finished, so dump remaining data from last command, if any
				if offset > 0:         # 
					if args.v > 1:
						print_data(offset)
						offset = 0
	...
	# loop without having a chance to print this. kind of ugly.
	if offset > 0:
		if args.v > 1:
			print_data(offset)
			offset = 0
